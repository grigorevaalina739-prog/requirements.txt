...
async def notify_deadline_changed(bot: Bot, assignee: str, title: str, old_deadline: str,
                                   new_deadline: str, task_id: int, changed_by: str = ""):
    """Уведомляет ответственного об изменении срока задачи."""
    if not assignee:
        return
    assignees = [a.strip() for a in assignee.split(",") if a.strip()]
    for name in assignees:
        ...
        if not user:
            continue
        try:
            who = f"\n✏️ Изменил: {changed_by}" if changed_by else ""
            await bot.send_message(
                user["telegram_id"],
                f"📅 *Изменён срок задачи!*\n\n"
                f"📋 *Задача:* #{task_id} {title}\n"
                f"🔁 *Было:* {old_deadline or '—'}\n"
                f"➡️ *Стало:* {new_deadline or '—'}"
                f"{who}",
                parse_mode="Markdown"
            )
        except Exception as e:
            logger.error(f"Ошибка уведомления об изменении срока {name}: {e}")


async def notify_task_completed(bot: Bot, assignee: str, title: str, task_id: int, completed_by: str = ""):
    """Уведомляет ответственного о завершении задачи."""
    if not assignee:
        return
    assignees = [a.strip() for a in assignee.split(",") if a.strip()]
    for name in assignees:
        ...
        if not user:
            continue
        try:
            who = f"\n👤 Завершил: {completed_by}" if completed_by else ""
            await bot.send_message(
                user["telegram_id"],
                f"✅ *Задача завершена!*\n\n"
                f"📋 *Задача:* #{task_id} {title}"
            ...
            logger.error(f"Ошибка уведомления о завершении {name}: {e}")
...
async def cmd_mytasks(message: Message):
    ...
    my_tasks = [t for t in all_tasks if name.lower() in (t.get("assignee") or "").lower()
                and t.get("status") != "Выполнена"]
    if not my_tasks:
        await message.answer(f"✅ У вас нет активных задач, *{name}*.", parse_mode="Markdown")
        return
