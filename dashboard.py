...
">{p['name']}</a>"""
    ...
    html = (
        ...
        "<style>\n"
        ":root{--bg:#f0f4fa;--surface:#ffffff;--surface2:#f8faff;--accent:#4f7ef7;--accent2:#6366f1;--red:#e05c5c;--emerald:#16a37a;--amber:#d97706;--purple:#7c3aed;--muted:#64748b;--text:#0f172a;--text2:#475569;--border:#e2e8f0;--shadow:0 1px 3px rgba(0,0,0,.07),0 1px 2px rgba(0,0,0,.04);}\n"
        ...
        ".topbar{background:linear-gradient(160deg,#ffb3b3 0%,#ff6b6b 45%,#e83232 100%);backdrop-filter:blur(20px);border-bottom:1px solid rgba(255,255,255,.1);padding:0 32px;height:60px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:100;}\n"
        ...
        ".score{background:rgba(255,255,255,.9);border:1px solid rgba(255,255,255,.95);box-shadow:0 2px 8px rgba(0,0,0,.1);border-radius:14px;padding:12px 16px;text-align:center;min-width:80px;transition:transform .2s;}\n"
        ...
        ".att-card{background:rgba(255,255,255,.6);border:1px solid rgba(255,255,255,.75);border-radius:14px;padding:16px;margin-bottom:18px;backdrop-filter:blur(8px);}\n"
        ...
        ".srch input{width:100%;padding:7px 12px 7px 32px;border:1px solid var(--border);border-radius:20px;font-size:12px;background:rgba(15,22,41,.8) !important;color:#f1f5f9 !important;-webkit-text-fill-color:#f1f5f9 !important;outline:none;transition:all .15s;}\n"
        ...
        ".task-row{display:block;margin:6px 0;border-radius:10px;border:1px solid var(--border);padding:14px;background:white;}"
        ".task-row td{display:block;padding:0;margin-bottom:10px;font-size:14px;line-height:1.6;}"
        ".task-row td:last-child{margin-bottom:0;}"
        ".task-row td strong{display:block;font-weight:600;color:var(--text);margin-bottom:3px;}"
        ".ctrl-bar{padding:12px;}"
        ".pill{padding:8px 13px;font-size:13px;min-height:40px;}"
        ".srch input{padding:10px 35px;font-size:15px;min-height:44px;}"
        ".btn-reset{min-height:40px;padding:8px 14px;font-size:13px;}"
        "select{min-height:44px;font-size:15px;padding:8px 12px;}"
        ".kpi{padding:14px;}"
        ".kpi-num{font-size:24px;}"
        ".hero{padding:24px 14px;}"
        ".hero-h2{font-size:20px;}"
        ".hero-sub{font-size:12px;margin-top:6px;}"
        ".score{min-width:70px;padding:10px 12px;}"
        ".score .val{font-size:18px;}"
        ".score .lbl{font-size:11px;}"
        ".ai-card{padding:12px;margin:14px 14px;}"
        ".ai-text{font-size:13px;}"
        ".act-btn{width:36px;height:36px;font-size:14px;}"
        ".row-acts{opacity:1;}"
        ".prog-card{padding:14px;}"
        ".prog-title{font-size:13px;}"
        ".prog-pct{font-size:24px;}"
        ".att-card{padding:12px;}"
        ".sb-sec{margin-bottom:14px;padding:0 7px;}"
        ".sb-title{font-size:11px;padding:0 7px;}"
        "}\n"
        ...
        ".tb-btn{padding:6px 10px;font-size:11px;min-height:36px;}"
        ".pill{min-height:36px;padding:6px 11px;font-size:12px;}"
        ".hero{padding:18px 10px;}"
        ".hero-h2{font-size:18px;}"
        ".content{padding:10px;}"
        ".task-row{margin:4px 0;padding:12px;font-size:13px;}"
        ".kpi-num{font-size:22px;}"
        ".kpi-label{font-size:10px;}"
        ".score{min-width:60px;padding:8px 10px;font-size:11px;}"
        ".score .val{font-size:16px;}"
        ".ai-card{margin:10px 0;padding:10px;}"
        ".prog-title{font-size:12px;}"
        ".prog-pct{font-size:20px;}"
        ".att-card{padding:10px;margin-bottom:10px;}"
        ".sb-sec{padding:0 5px;margin-bottom:10px;}"
        ".srch input{font-size:14px;min-height:40px;padding:8px 30px;}"
        "}\n"
        "</style>\n"
        "</head>\n<body>\n"
    )
    ...
    html += (
        ...
        "      <div class=\"cmd-item\" onclick=\"location='/agent'\" >🤖 Агент</div>\n"
...
async def agent_page(request):
    ...
    managers = get_managers() + ["Турбина Е."]
...
<div class="chat-area">
  ...
  <div class="input-wrap">
    ...
    <div class="input-hints">
      ...
      <span class="hint-chip" onclick="useTemplate('Мустафина провести собеседование с кандидатом')">👤 HR задача</span>
    </div>
  </div>
</div>
...
async def managers_page(request):
    ...
    if msg:
        msg_html = (
            f'<div style="background:#DCFCE7;color:#166534;padding:12px 16px;'
            f'border-radius:8px;margin-bottom:16px;font-size:14px;">✅ {msg}</div>'
        )
    if error:
        msg_html += (
            f'<div style="background:#FEE2E2;color:#991B1B;padding:12px 16px;'
            f'border-radius:8px;margin-bottom:16px;font-size:14px;">⚠️ {error}</div>'
        )
    ...
    if managers:
        for name in managers:
            ...
            rows_html += (
                ...
                f'  <a href="/managers/delete?name={name}" class="mdel"'
                f'     onclick="return confirm(\'Удалить сотрудника {safe_name}?\')">🗑 Удалить</a>'
                f'</div>'
            )
    else:
        rows_html = (
            '<div style="text-align:center;padding:40px;color:#94A3B8;">'
            'Список сотрудников пуст</div>'
        )
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Сотрудники — MINISO</title>
<style>
  * {{box-sizing:border-box;margin:0;padding:0;}}
  body {{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#F9FAFB;color:#111827;min-height:100vh;}}
  .topbar {{background:#1E293B;color:white;padding:16px 32px;display:flex;justify-content:space-between;align-items:center;}}
  .topbar h1 {{font-size:20px;font-weight:600;}}
  .topbar a {{color:#94A3B8;text-decoration:none;font-size:14px;transition:color .15s;}}
  .topbar a:hover {{color:white;}}
  .wrap {{max-width:640px;margin:32px auto;padding:0 16px;}}
  .card {{background:white;border-radius:12px;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.05);}}
  .card h2 {{font-size:16px;font-weight:600;margin-bottom:14px;color:#1E293B;}}
  .addform {{display:flex;gap:10px;margin-bottom:24px;}}
  .addform input {{flex:1;padding:10px 14px;border:1px solid #E5E7EB;border-radius:8px;font-size:14px;outline:none;transition:border-color .15s;}}
  .addform input:focus {{border-color:#3B82F6;}}
  .btn-add {{padding:10px 20px;background:#3B82F6;color:white;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;transition:background .15s;}}
  .btn-add:hover {{background:#2563EB;}}
  .mrow {{display:flex;justify-content:space-between;align-items:center;padding:12px 14px;border:1px solid #E5E7EB;border-radius:8px;margin-bottom:8px;transition:background .15s;}}
  .mrow:hover {{background:#F9FAFB;}}
  .mname {{font-size:14px;color:#1E293B;font-weight:500;}}
  .mdel {{padding:6px 12px;background:#FEE2E2;color:#DC2626;border-radius:6px;font-size:12px;font-weight:600;text-decoration:none;transition:background .15s;}}
  .mdel:hover {{background:#FCA5A5;color:#7F1D1D;}}
  .counter {{font-size:12px;color:#64748B;margin-top:16px;text-align:center;}}
</style>
</head>
<body>
<div class="topbar">
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <form class="addform" method="POST" action="/managers/add">
      <input type="text" name="name" placeholder="Например: Иванов И." required autofocus>
      <button type="submit" class="btn-add">Добавить</button>
    </form>

    <h2>Текущий список ({len(managers)})</h2>
    {rows_html}
    <div class="counter">Изменения сразу видны в дашборде и в Telegram-боте</div>
  </div>
</div>
</body>
</html>"""
...
@routes.post("/managers/add")
async def managers_add(request):
    """Обработчик добавления сотрудника."""
    data = await request.post()
    name = (data.get("name") or "").strip()
    if not name:
        raise web.HTTPFound("/managers?error=Пустое имя")
    if len(name) > 100:
        raise web.HTTPFound("/managers?error=Слишком длинное имя")
    ok = add_manager(name)
    if ok:
        raise web.HTTPFound(f"/managers?msg=Добавлен: {name}")
    else:
        raise web.HTTPFound(f"/managers?error=Сотрудник {name} уже есть в списке")


@routes.get("/managers/delete")
async def managers_delete(request):
    """Удаление сотрудника (GET, чтобы работало из простой ссылки)."""
    name = (request.query.get("name") or "").strip()
    if not name:
        raise web.HTTPFound("/managers?error=Не указано имя")
    ok = delete_manager(name)
    if ok:
        raise web.HTTPFound(f"/managers?msg=Удалён: {name}")
    else:
        raise web.HTTPFound(f"/managers?error=Сотрудник {name} не найден")
...
async def calendar_page(request):
    ...
    participants_list = ",".join(get_managers())
